...
The key challenge lies in parameterizing the neural model such that it provides meaningful guidance during the search. Specifically, the network is trained to assign a score to each node---that is, to each feasible extension of a current partial solution---based on features extracted from the corresponding partial solution and the associated subproblem.

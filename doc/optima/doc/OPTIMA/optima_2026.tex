...
\end{algorithm}
 ...
 \textcolor{red}{TODO: Came up to here!}
 The process repeats until a time limit $t_{\max}$ is reached (set to 2 CPU hours). The algorithm returns the best weight vector $w^\ast$, corresponding to the neural network $\mathcal{N}(w^\ast)$ that induces the most effective heuristic $h_{w^\ast}$.
 
 Our experiments use the following parameters for BRKGA:  $n_{\text{pop}}=20$, $n_{\text{elite}}=1$, $p_{\text{bias}}=0.5$, and $n_{\text{mutants}}=7$. A visualization of the learning process is provided in Fig.~\ref{fig:ga_imsbs}.
...
The resulting dataset, denoted \textsc{Real}, consists of 20 instances (10 from each benchmark set). All sequences have length 600, while the number of sequences varies from $m=10$ to $m=200$.
 ...
 All benchmark sets with the obtained neural network configurations are freely available at   
\url{https://github.com/markodjukanovic90/VGLCS\_Neural\_Heuristics}. 
 
\subsection{Tuning Process}

The parameters of \textsc{Imsbs} are tuned on the benchmark set \textsc{Random} using \texttt{irace}~\cite{perez2014analysis}, with a total budget of 5000 runs (each limited to 30 minutes). For tuning, one instance from each group is selected, resulting in 32 training instances in total. All remaining (less sensitive) parameters are fixed to the values reported  in~\cite{djukanovic2026_eurocast}. The produced configuration for \textsc{Imsbs} is $\beta=2000$, $h=\text{UB}$, and $\#\texttt{imbs\_iters}=5000$.
...
To assess statistical significance, we perform a one-sided (one-sided) Wilcoxon signed-rank test on paired instances (Table~\ref{tab:statistical_test}).
...
\begin{itemize}
	\item  Out of 32 instances, \textsc{Limsbs-ensemble} achieves better performance over \textsc{Imsbs} in 20 cases, ties occur in 8 cases, and in 4 cases the winner is \textsc{Imsbs}.
...
Overall, the results demonstrate that \textsc{Limsbs-ensemble} provides a consistent improvement in solution quality over \textsc{Imsbs}, while maintaining comparable computational efficiency. This makes it a preferable choice when solution quality is the primary objective.

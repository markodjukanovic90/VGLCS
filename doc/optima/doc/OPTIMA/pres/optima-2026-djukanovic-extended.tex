...
\begin{frame}{LCS: Literature \& Problem Variants }
 ...
 \vspace{0.2cm}
  \item When $m$ arbitrary large -- \textcolor{red}{$\mathcal{NP}$-hard}:
   \begin{itemize}
...
\begin{frame}{The gapped LCS problem (Peng and Yang, 2012)}
   \begin{definition}[A gap sequence]
       ...
       $G_s \colon \{1, \ldots, |s|\} \mapsto \mathbb{N}$. A pair\textcolor{red}{$(s, G_s)$} is called a {gap sequence}.
   ...
   \begin{definition}[A gapped subsequence]
         ...
         \begin{itemize}
         	\item consecutive embedding positions respect gap distances (no more than the \textcolor{red}{given number of letters} inbetween respected)
...
\begin{itemize}
  ...
  \item We learn \textcolor{blue}{a novel node evaluator}: a function assigning a promise score to a partial solution
\end{itemize}
\vspace{0.15cm}
...
\small
For a state $\textbf{v}=(\mathbf p^v,l^v)$ ($p^v$: vector of suffix positions of subproblem to be solved, $l^v$: lenght of partial solution), build a size-independent features:
\vspace{0.2cm}
\begin{columns}[T,totalwidth=\textwidth]
...
\begin{frame}{How close are we to the exact DP-1 solution?}

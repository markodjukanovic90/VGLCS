...
\begin{frame}{Root-based state graph formulation: example}
    ...
    Given $s_1=$\texttt{\fbox{A}BCA},  $s_2=$\texttt{\fbox{A}CAB}, assuming  $G_1=G_2=1$, match $p^L=(1, 1)$; \\ \vspace{0.3cm} \textsc{SubSpace}($(1, 1)$): \textcolor{red}{\textbf{partial solutions start with \texttt{A} at} $(1, 1)$}
...
\begin{frame}{An issue with the root-state-space formulation: disconnected components}
	...
	$\Longrightarrow$ Iterative Multi Source Beam Search (IMSBS) approach. 
\end{frame}
...
\begin{itemize}
  \item We use a feed-forward multilayer perceptron (MLP).
...
\begin{itemize}
  ...
  \item Fitness = average subsequence length found.
\end{itemize}
\end{block}
\[
...
\begin{frame}{Learning weights with BRKGA}
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.50\textwidth}
\begin{itemize}
...
\[
 r(v)=\alpha\,r_{h_{w^*}}(v)+(1-\alpha)\,r_{UB}(v).
\]
...
\begin{itemize}
\item tuned by \texttt{irace}  (parameters from the literature)
\end{itemize}
...
\\
	...
	\begin{block}{Conclusion}
		\begin{itemize}
			\item 	Learning an improved guidance promising approach
		    \item Low alphabet diversity, despite improved, still requires stronger exploration/richer features.
		\end{itemize}
	\end{block}

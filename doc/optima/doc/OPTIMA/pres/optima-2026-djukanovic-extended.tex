...
\begin{frame}{An issue with the root-state-space formulation: disconnected components}
	...
	\begin{itemize}
		\item  Generation of longest solution for a root (sub-)space is generally hard $\Rightarrow$ need an approx. approach (beam search -- limited BFS)
		\item  The root-state-space formulation may involve disconnected components generation  $\Rightarrow$ needs iterative beam search to explore all space 
	\end{itemize} \vspace{0.3cm}
...
\node[draw,rounded corners,fill=orange!10,align=center,right=of expand] (score) {score each node\\$h_w(u)$};
...
\begin{itemize}
  ...
  \item Evaluate $w$ by running IMSBS on training instances.

...
void compute_features(std::vector<Node*>& V_ext){
    ...
    for(Node* node : V_ext){
        ...
        for(int i = 0; i < pL_v.size(); ++i) pL_v[i] = pL_v[i] / node->inst->S[i].size();
        for(int i = 0; i < lv.size(); ++i) lv[i] = lv[i] / node->inst->R[i].size();
...
double BS(double t_lim, int beta, Instance* inst, MLP& neural_network, bool training)
{
    ...
    std::vector<Node*> to_delete; // to be deleted after the execution of BS
    ...
    while(!beam.empty()) {
        ...
        for(const auto & node: beam) {  
         ...
         // Use the neural network for sorting V_ext
         // First calculate features and then call the neural network for every node in V_ext
        ...
        if(differentiate_ties){
            if(V_ext.size() > beam_size){ //look for ties
                ...
                if(limit_h == V_ext[beam_size]->heuristic_value){ //there are ties
                    ...
                    //calculate secondary_heuristic_value for the nodes in tied_V_extd
    ...
    for(Node* node : to_delete) delete node; //free memory 
    return l_best;
}

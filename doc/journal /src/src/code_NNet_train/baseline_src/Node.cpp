...
Node::Node(Instance *instance, std::vector<int>& left, std::vector<int>& ppos, std::vector<int>& rpos, Node* parent)
{
    ...
    HeuristicFunction *hf  = nullptr;
    HeuristicFunction *hf_secondary  = nullptr;

    switch(h_heuristic)
    {
      case UB_1:  hf = new UB1(); break; 
      case UB_2:  hf = new UB2(); break;
      case UB_min:  hf = new UB();  break; 
      case G:  {double greedy = this->greedy_function(); this->heuristic_value = greedy;} break;
      case Prob:  hf= new UB();  break;  //used for sorting out first V_ext nodes 
      default: break;//std::cerr << "Not the right decision regarding the choice of search heuristic..." ;
    }

    if (h_heuristic <= 2) // UB or UB1 or UB2 
    {
        this->heuristic_value = binary_op(left, ppos, rpos,  0, instance, hf);  // primary criterion 
        hf_secondary  = new R_min();//  
        this->secondary_value = binary_op(left, ppos, rpos,  0, instance, hf_secondary); // secondary criterion always involved by R_min (the larger, the better)
        // correction of UB_1 value in A*?? or correcting UB -- the first option for now kept  
    }
    if(h_heuristic == Prob) // Prob-guidance
    {
         this->heuristic_value = binary_op(left, ppos, rpos,  0, instance, hf); 
         hf_secondary  = new R_min();   //UB has been 
         this->secondary_value = binary_op(left, ppos, rpos, 0, instance, hf_secondary); // left[0]-1 >= 0 ?this->inst->S[0][left[0]-1] : 0
...
void Node::set_up_prob_value(int k)
{
     ...
     this->heuristic_value = binary_op(std::get<0>(this->position), std::get<1>(this->position), std::get<2>(this->position), 0, this->inst, hf);  
...
std::vector<int> Node::export_solution()
{
    ...
    //std::cout << next_in_s0<< std::endl;
    Node* node = this->parent; // tha paret node of the node this

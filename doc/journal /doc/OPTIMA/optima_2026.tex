...
\end{figure}
	 ...
	 The amount of nodes to further pursue is controlled by  beam width ($\beta$)---the level of greediness---and a heuristic guidance ($h$) to select candidate nodes---as especially sensible algorithmic parameter. The heuristic used in~\cite{djukanovic2026_eurocast} is given as $\texttt{UB}(v)= l^v + \sum_{a \in \Sigma }  \min_{i=1,\ldots, m} |s_i[p^{L}_i, |s_i|]|_a$. For each letter $a\in \Sigma$, it first evaluates an upper bound on the number of its appearance in each residual sequence that corresponds to node $v$. These values are then sum up to generate a real upper bound value for the length of the LCS of the subproblem associated to node $v$. 
...
% \textcolor{red}{TODO: To adapt the content; it should be more clear from the beginning what to be optimized and what means a learning heuristics here ... mentioning F1, F2, F3 feature configurations only in Experimental section ...}
 ...
 Neural networks (NNs) present a well-known machine learning model~\cite{gurney2018introduction} that has shown its applicability in a wast majority of fields~\cite{bishop1994neural}. These models include basic multi-layer perceptron towards more complex constitutional and recurrent neural networks of different architectures~\cite{mienye2024recurrent}.  To  keep it simple but efficient, we will consider only the fully-connected (multi-)layer NNs  to derive an alternative heuristic to the known hand-crafted versions  for guiding the search process of the IMSBS, see~\cite{djukanovic2026_eurocast}.  For doing so, the major issue is to properly parameterize the neural network, represented by set of weights assigned to its edges to induce a data-driven neural heuristic. It will  be learned to predict a score for each node based on a set of extracted features extracted for associated partial solution and its subproblem.
 
 In the context of classical neural networks, this learning process is performed by alternating between forward and backward propagation, where the later is used to weights adaptation\  w.r.t.\  error minimization (of a loss function) on a training dataset. The process of adapting weights is usually executed by employing iterative numerical procedures such as (stochastic) gradient descent~\cite{amari1993backpropagation} or the versions thereof. % using the power of gradient direction in quickly reaching local optima thus decreasing the overall loss w.r.t. known target values, hoping to generalize well.  %on unseen data. \\
 As we operate in the combinatorial space of an \textsc{NP}-hard problem, we do not expect the target values are known in advance, or there will be known to some extent but only for small-to-middle sized instances. This limits the application of gradient descent algorithms in training weights on such environments.  We approach here differently by employing population-based meta-heuristic approach dealing with various NN configurations, which are evolved towards local optima.   Fitness values of specific NN configurations are evaluated in a training instances by solving each of them using a robust (beam search) approach guided by the induced data-driven heuristic. As next we give details on the learning process. \\
 
  
\textbf{Predefined Architecture of NN}. The heuristic is represented by a \textit{multi-layer perceptron} built of input of multiple neurons,  output layer of one node, and a number of hidden layers with user-defined units per layer.  The input layer received a set of numerical values associated to the features of  subproblem to be processed. The output of NN refers to how much the considered subproblem is ``promising''. 
In each layer, to catch up  non-linearity between input features, the following activation functions are tested: \texttt{hyperbolic tangent}, \texttt{ReLU}, and \texttt{sigmoid}. For each layer $l$, forward propagation is defined as $
 \mathbf{z}^{(l+1)} = W^{(l)} \mathbf{a}^{(l)} + b^{(l)}, \quad
 \mathbf{a}^{(l+1)} = \phi(\mathbf{z}^{(l+1)}),
 $
 where $W^{(l)}$ and $b^{(l)}$ are the weight matrix and bias vector, respectively, and $\phi$ denotes the activation function. The final \textbf{z}-value represents the heuristic score.    All weights and biases are flattened into a {single vector} $w=\texttt{flatten}(W)$, which forms the chromosome of a GA individual. \\
  \textbf{Fitness Evaluation of NN configurations}. Given a candidate weight vector $w$, we perform:  1) Load of the weights into a FFNN architecture $\mathcal{N}$ of predefined architecture;
 	2) Executing the IMSBS on each instance from a training set $\mathcal{T}$ utilizing the neural heuristic to guide the search of IMSBS in the most sensitive forward iterations of BS; and 3)  Recording size of the best sequence obtained for each instance. The fitness value of candidate vector $w$ over the pre-defined NN architecture $\mathcal{N}$ 
  is computed as the average solution quality over all training instances. Formally, the learning problem is defined as:
 ...
 \end{equation}
 where $h_w$ is the heuristic induced by neural network $\mathcal{N}(w)$, and  $f(\cdot)$ is a problem-specific objective function---representing length of longest  solution found by an IMSBS guided by $h_w$. Validation performance score is computed analogously on a separate dataset $\mathcal{V}$. %and is used only for reporting.  
 
 
\textbf{Feature Extraction}. To guide the search process, each node is mapped to a fixed-length numerical feature vector capturing both the current state in construction phase and the structure of induced subproblem to solve. These features are designed to be instance-independent. %and comparable across problem sizes.
 ...
 From this normalized position vector, four statistical descriptors are extracted: $\max(\textbf{F})$, $\min(\textbf{F})$, $\texttt{mean}(\textbf{F})$, and standard deviation $\sigma(\textbf{F})$. These quantities characterize, respectively, the most advanced and least advanced sequences, the average depth of the search, and the imbalance of progress among sequences. In addition, {the  length of currently constructed partial subsequence} $l^v$ is included as another feature. 
 ...
 These statistics for gap constrains summarize the remaining flexibility of the instance: large average or maximum gaps indicate greater freedom for future matches, whereas small minimum gaps and large variability typically signal tighter constraints and increased difficulty.
 Additional features involved are  $|\Sigma|$ and the value of $m$ (number of input sequences), providing global structural information about the instance and help the learning model adapt to varying problem scales. Note that we additionally probed with involving more features such as length of input sequences, however with a negative impact on solution quality.  %of the learning approach. 
 ...
 Overall, the resulting representation combines information about (i) the current construction progress, (ii) the remaining gap-constraint landscape, and (iii) global instance properties, yielding 11 features. This compact yet expressive encoding enables effective learning-based guidance of the search for high-quality  subsequences. 
 
 \textbf{Optimize for high-quality configurations}. We opt for a Genetic algorithm, in particular its popular version called the \textit{Biased Random Key Genetic Algorithm} (BRKGA) to solve the problem given in Eq.~(\ref{eq:training_problem_optimization}).  The optimization process follows a population-based evolutionary scheme, described in Algorithm~\ref{alg:learning_mechanism}. Before we briefly explain the main workflow of the algorithm, first a few important notes.  Each individual is initialized by sampling weights uniformly from a predefined interval [$-q, q$] ($q$ is fixed to 1).
  %\textit{Elitism} is included by copying a fixed number of top-performing individuals into the next generation. \textit{Mutation} is performed so that several weakest individuals  in each generation are generated completely at random (random immigrants). This mechanism promotes exploration and maintains population diversity.  Concerning \textit{offspring generation} in BRKGA, one parent is selected from the elite set and one from the non-elite set. Each gene is inherited from the elite parent with a predefined probability (this is fixed to 0.5).  The new population consists of elites, mutants, and offspring. 
...
Given are multi-layer perception architecture $\mathcal{N}$, parameters of IMSBS, and parameters of BRKGA to be involved in the fitness calculation of NN configurations.  %weights that induce high-quality heuristics.
 ...
 fashion, terminating the process whenever the validation value
 decreases, as a criterion for detecting  over-fitting. Otherwise, after generating all individuals, the population is updated  ($P \leftarrow P'$). The process is repeated until the time limit $t_{\max}$ is reached---here fixed to 2 CPU hours.  
The algorithm returns the best solution $w^\ast$, corresponding to the neural network weights that induce the most effective search heuristic $h_w=\mathcal{N}(w)$ on the training set.  As a reasonable optimizer, the BRKGA setting used for training takes the values  $n_{pop}=20, n_{elites}=1, p_{bias}=0.5, p_{mutants}=7$. Visualization of the learning process is depicted in Fig.~\ref{fig:ga_imsbs}.  

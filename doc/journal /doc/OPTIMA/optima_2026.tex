...
We consider two types of benchmark sets.

\emph{Synthetic instances.}
The first set, denoted \textsc{Random}, is borrowed from~\cite{djukanovic2026_eurocast}. For each combination of parameters $n \in \{50, 100, 200, 500\}$, $m \in \{2, 3, 5, 10\}$, and $|\Sigma| \in \{2, 4\}$, 10 instances are generated, resulting in a total of 320 problem instances.

\emph{Biologically motivated instances.}
To complement synthetic data, we introduce a new set of instances with data-driven gap constraints derived from biological sequence properties. The goal is to model context-dependent positional importance using thermodynamic stability information, inspired by sequence alignment scoring and weighted subsequence models.
...
\]
where $\Delta G$ denotes the free energy associated with the dinucleotide pair. The corresponding weights are given by:
...
To ensure consistency with the directional nature of gap constraints, we define a \emph{causal} positional contribution:
\[
f^s_k =
\begin{cases}
	f(s[k-1], s[k]), & k > 1, \\
...
The resulting dataset, denoted \textsc{Real}, consists of 20 instances (10 from each benchmark). All sequences have length 600, while the number of sequences varies from $m=10$ to $m=200$.
...
The parameters of \textsc{Imsbs} are tuned on the benchmark set \textsc{Random} using \texttt{irace}~\cite{perez2014analysis}, with a total budget of 5000 runs (each limited to 30 minutes). The resulting configuration is: $\beta=3000$, $h=\text{UB}$, and $\#\texttt{imbs\_iters}=5000$. For tuning, one instance from each group is selected, resulting in 32 training instances in total. All remaining (less sensitive) parameters are set according to~\cite{djukanovic2026_eurocast}.

For a fair comparison, the proposed \textsc{Limsbs-ensemble} inherits the same \textsc{Imsbs} parameter configuration. The learning component is tuned by exploring different neural network architectures, including two-layer and three-layer multi-layer perceptrons with 5 or 10 neurons per layer.

To reduce the computational burden, we restrict the activation function to be identical across all layers, selecting from $\{\texttt{tanh}, \texttt{ReLU}, \texttt{sigmoid}\}$. To analyze the sensitivity with respect to feature design, we define three feature configurations:
\begin{itemize}
...
After extensive empirical evaluation, the following configurations are selected for the \textsc{Random} benchmark:
\begin{itemize}
...
The ensemble parameter is set to $\alpha = 0.45$, except for $m=3$ where $\alpha=0.5$, based on grid search.

For training the neural heuristic, we use 32 instances for the training set $\mathcal{T}$ and 32 for the validation set $\mathcal{V}$, selecting one instance per group. For each value of $m$, this results in 8 instances used in the learning process (for both, training and validation purposes).

For the \textsc{Real} benchmark, we employ the same NN architecture as in the $m=10$ case from \textsc{Random}. This choice allows us to evaluate the robustness of the learned heuristic on structurally different instances with a larger number of sequences ($m \geq 10$). The \textsc{Imsbs} parameters remain unchanged, and follows those used to test on the set \textsc{Random}.  

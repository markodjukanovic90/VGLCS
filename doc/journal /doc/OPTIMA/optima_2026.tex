...
Table~\ref{tab:preliminary_results} reports the average results over 10 instances per group. The table is organized into two parts: instance characteristics and performance metrics for \textsc{Limsbs-ensemble} and \textsc{Imsbs}.
...
The results indicate a statistically significant improvement of \textsc{Limsbs-ensemble} over \textsc{Imsbs}. Out of 32 instances, \textsc{Limsbs-ensemble} achieves better performance in 20 cases, ties in 7 cases, and is outperformed in only 5 cases.
...
Table~\ref{tab:real_results} summarizes the results, including solution quality, runtime ($t$ in seconds), and reference LCS values from the literature (LCS$^*$). The following observations can be made:

\begin{itemize}
	\item \textbf{Solution quality.} \textsc{Limsbs-ensemble} outperforms \textsc{Imsbs} in 15 out of 20 instances, with the remaining 5 resulting in ties. No instance favors \textsc{Imsbs}, indicating consistent superiority of the proposed method.
	
	\item \textbf{Magnitude of improvement.} The average solution quality increases from 9.9 (\textsc{Imsbs}) to 11.8 (\textsc{Limsbs-ensemble}), corresponding to an average improvement of approximately 19\%. Median values (5 vs.\ 4) confirm that gains are not driven by outliers.
	
	\item \textbf{Instance difficulty.} The improvements are more pronounced on challenging instances (e.g., virus datasets), where substantial gains are observed. Ties typically occur on simpler instances with low objective values.
	
	\item \textbf{Runtime.} The computational effort of both approaches is comparable. While \textsc{Imsbs} is slightly faster on average, the differences are not consistent, and \textsc{Limsbs-ensemble} is competitive in most cases.
	
	\item \textbf{Impact of gap constraints.} Compared to unconstrained LCS values from the literature, the introduction of gap constraints significantly reduces achievable subsequence lengths. This effect is particularly strong for large $m$ (e.g., $m \geq 60$), where feasible solutions become highly restricted.
\end{itemize}

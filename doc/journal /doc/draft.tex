...
\end{figure}
 ...
 \section{Further ideas} (TODO: To be exploited)
 
 Using only the mean optimizes average performance, ignores failures, and can produce unstable heuristics.  In COPs, robustness is often more important than average performance.
 
 Fitness function replacement:
 \begin{itemize}
 	\item  Option A (robust \& simple)   Fitness = 0.7 $\cdot$ mean + 0.3 $\cdot$ 90th-percentile
    \item  Option B (very strong in COPs) Fitness = mean regret + 0.2 * std\_dev
   \item Option C (robust learning): Fitness = 90th percentile\\
  \end{itemize}
 ...
 \textbf{ Multi-objective fitness}. Instead of a single scalar: Objective 1: average quality, Objective 2: worst-case, runtime
  
  Then use:  Pareto ranking (common in evolutionary algorithms), particularly natural for Biased Random-Key Genetic Algorithms (BRKGA). 

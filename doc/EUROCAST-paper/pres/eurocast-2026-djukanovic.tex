...
\begin{frame}{Literature \& Motivation}
   ...
   \begin{itemize}
       \item  Peng and Yang (2014): studied the problem with $m=2$ (poly-version): \textbf{Three dynamic programming} approaches (basic and sparse, sparse + advanced data-structures)
       \item  Manea et al. (2024): Complexity bounds,  (parameterised) complexity analysis investigated
   \end{itemize} \vspace{0.3cm}
   
   Motivation:
   \begin{itemize}
       \item  \textbf{Bioinformatics}: applications in DNA and protein analysis where variable structural distances between residues must be respected
       \item \textbf{Time-series analysis}:  especially in settings where events are required to occur within specified temporal delays~(Lainscsek et al. (2015))
   \end{itemize}
...
\begin{frame}{Root-based state graph formulation: rough idea}
  ...
  \fbox{\textcolor{red}{\textbf{Crucial: selection of position $r$ for root node 
   (corresp. to empty partial sol.)}}} $\Rightarrow$ possibly \textbf{(exponentially) many root nodes}. \\ State graphs space induced by root node $(r, 0)$ we denote by \emph{Space(r)}.
\end{frame}
...
\begin{frame}{An issue with the root-state-space formulation: example}
   ...
   \item  A search strategy that expands states from a single initial root may fail to reach feasible and even optimal solutions.     \end{itemize}
   \begin{example}
   		$
   		S = \{ s_1 = \texttt{ATGGAAAA},\; s_2 = \texttt{ATCCAAAA} \},
   		$
   		with gap constraints $G_{s_1} = G_{s_2} = 1$. In this instance, any state with position vector $\mathbf{p}^L = (5,5)$ cannot be reached  from the initial state $((1,1),0)$ by standard directed transitions. 

  \end{example}
  
   The optimal common subsequence \texttt{AAA} is unreachable from the basic root $((1,1),0)$,  therefore missed by traditional search strategies.

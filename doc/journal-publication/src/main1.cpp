...
void read_parameters(int argc,char** argv){
    ...
    while (iarg < argc) {
        ...
        for(int i = 0; i < hidden_layers; ++i)
            ...
            if(feature_config == 1) num_features = 9; //p^{L,v}, l^v and length of partial sol (used for trainings 4)
            else if(feature_config == 2) num_features = 10; // plus alphabet size (used for trainings 1,2)
            else if(feature_config == 3) num_features = 12; //plus num of input and restricted strings  
            else if(feature_config == 4) num_features = 14; //plus num of and length of input and restricted strings (used for trainings 3)
        }
